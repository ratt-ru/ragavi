...
def complex_values_x(complex_values_d):
    return xr.DataArray(data=complex_values_d, coords={"x": np.arange(10)}, dims=("x",))
...
def uvw():
    return xr.DataArray(
        data=1 + da.array([[np.arange(5)], [np.arange(5)], [np.arange(5)]]).squeeze().T,
...
def test_phase(complex_values, pro):
    assert np.all(
        np.rad2deg(np.unwrap(np.angle(complex_values, deg=False)))
        == pro.calculate("phase").values
    )


def test_wrapped_phase(complex_values, pro):
    assert np.all(np.angle(complex_values, deg=True) == pro.phase(unwrap=False).values)


def test_uvdistance(uvw):
    assert np.all(
        np.sqrt(np.square(uvw.values[:, :-1]).sum(axis=1)) == Processor.uv_distance(uvw)
    )


def test_uvwavelength(uvw, freqs):
    assert np.all(
        (np.sqrt(np.square(uvw.values[:, :-1, np.newaxis]).sum(axis=1)) / (3e8 / freqs))
