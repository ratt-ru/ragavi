...
def vis_argparser():
    ...
    for grp in parser._action_groups:
        if "required" in grp.title.lower():
            grp.add_argument(
                "--ms", dest="msnames", nargs='+', type=str, metavar='', 
                default=[None],
        ...
        if "plot" in grp.title.lower():
            ...
            grp.add_argument(
                "-o", "--htmlname", dest="html_names", nargs='+', type=str, 
                metavar='', default=[None],
                help="Output HTML file name (without '.html')")

        if "selection" in grp.title.lower():
            grp.add_argument(
                "-b", "--baseline", dest="baselines", nargs='+', type=str,
                metavar='', default=[None],
            ...
            grp.add_argument(
                ...
                Default is all.""")
            grp.add_argument(
                "-dc", "--data-column", dest="data_columns", nargs='+',
...
def gains_argparser():
    ...
    for grp in parser._action_groups:
        if "required" in grp.title.lower():
            grp.add_argument(
                "-t", "--table", dest="msnames", nargs='+', type=str,
                metavar='', default=[None],
        ...
        if "selection" in grp.title.lower():
            ...
            grp.add_argument(
                ...
                default=[None], help="""Minimum time to plot [in seconds]. 
                Defaults to full range]""")
            grp.add_argument(
                "--t1", dest="t1s", nargs='+', type=float, metavar='',
                default=[None], help="""Maximum time to plot [in seconds]. 
                Defaults to full range""")
            grp.add_argument(
                "--taql", dest="taqls", nargs='+', type=str, metavar='',
                default=[None], help="TAQL where clause")
        
        if "plot" in grp.title.lower():
            grp.add_argument(
                "-y", "--yaxis", "-d", "--doplot", dest="yaxes", nargs='+',
                type=str, metavar='', default=["ap"],
